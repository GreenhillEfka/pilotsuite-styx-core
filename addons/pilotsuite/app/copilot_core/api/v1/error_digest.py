...
def error_digest():
    ...
    try:
        ...
        return jsonify({"ok": False, "error": "Invalid hours parameter"}), 400
    category_filter = request.args.get("category")
    severity_filter = request.args.get("severity")
    valid_severities = {"low", "medium", "high", "critical"}
    if severity_filter and severity_filter not in valid_severities:
        return jsonify({"ok": False, "error": f"severity must be one of: {', '.join(sorted(valid_severities))}"}), 400
    cutoff = time.time() - (hours * 3600)

    # Collect errors from dev logs ring buffer
    errors: list[dict] = []
    ...
    if dev_logs:
        for entry in dev_logs:
            ...
            source = entry.get("source", "") or entry.get("logger", "")
            ...
            if severity_filter:
                ...
                if sev_order.get(severity, 0) < sev_order.get(severity_filter, 0):
                    continue
...
def repair_suggestions():
    ...
    # If LLM is available and no pattern match, ask LLM
