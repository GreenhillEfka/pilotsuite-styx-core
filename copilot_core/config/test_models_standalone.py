"""Standalone tests for config hardening models (no HA dependency)."""
import sys
...
# Import directly from modules to avoid HA dependency
...
class TestAlarmConfig:
    ...
    def test_custom_days_validation(self):
        with pytest.raises(ValueError):
            AlarmConfig(repeat="custom")  # Missing custom_days
        with pytest.raises(ValueError):
            AlarmConfig(repeat="custom", custom_days=[7])  # Invalid day
