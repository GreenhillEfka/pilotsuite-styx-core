...
| `/api/v1/zone-automation/zones/<zone_id>/entities/read-model` | GET | Deterministic entity read-model for one zone (`?since=<revision>`, `compact=true` (supports true/false/1/0/yes/no/on/off) for reduced payload) |
