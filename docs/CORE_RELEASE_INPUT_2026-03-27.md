...
- Vor jedem echten Releaseversuch gilt strikt: `mache v15.2.0` → 5 Minuten warten → Release-Lock → `./scripts/export_15_2_0_release_status.sh` + `./scripts/check_15_2_0_release_status.sh` → `./scripts/check_15_2_0_release_gate.sh` erneut grün → Release workflow dispatch gemäß Runbook.
