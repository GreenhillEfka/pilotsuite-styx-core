...
# Module-level references set by setup_module
DevSurfaceService = None
DevLogEntry = None
ErrorSummary = None
_original_psutil = None
_mock_psutil = None


def setup_module():
    """Mock psutil and import dev_surface modules."""
    global DevSurfaceService, DevLogEntry, ErrorSummary, _original_psutil, _mock_psutil

    _original_psutil = sys.modules.get('psutil')
    _mock_psutil = Mock()
    _mock_psutil.Process.return_value.memory_info.return_value.rss = 134217728  # 128 MB
    sys.modules['psutil'] = _mock_psutil

    # Force (re)import of dev_surface modules with mock psutil
    for mod_name in list(sys.modules.keys()):
        if mod_name.startswith('copilot_core.dev_surface'):
            sys.modules.pop(mod_name, None)

    from copilot_core.dev_surface.service import DevSurfaceService as _DSS
    from copilot_core.dev_surface.models import DevLogEntry as _DLE, ErrorSummary as _ES

    DevSurfaceService = _DSS
    DevLogEntry = _DLE
    ErrorSummary = _ES
...
def teardown_module():
    ...
    # Clear modules that cached the mock psutil reference
    for mod_name in list(sys.modules.keys()):
        if mod_name.startswith('copilot_core.dev_surface') or mod_name.startswith('copilot_core.monitoring'):
            sys.modules.pop(mod_name, None)
