...
| `/api/v1/notifications` | GET, POST | Read-only Notification-Feed plus minimaler Root-Create-Alias mit Read-after-write-Konsistenz und historischer `data`-/`channel`-/`source`-Payload-Kompatibilität plus Integer-`priority`-Normalisierung ohne Delivery-Reintro |
| `/api/v1/notifications/send` | POST | Minimaler Notification-Create-Write für genau eine API-Notification mit Read-after-write-Konsistenz und demselben historischen `data`-/`channel`-/`source`-Kompatibilitätsscope plus Integer-`priority`-Normalisierung |
...
- ✅ **Notifications Root Create Alias** — `POST /api/v1/notifications` nutzt bewusst denselben minimalen API-Write-Scope wie der Send-Pfad, akzeptiert zusätzlich historische `data`-, `channel`- und `source`-Payload-Hints plus Integer-`priority`-Werte und hält Feed-, Digest- und Stats-Sicht per Read-after-write synchron
- ✅ **Notifications Send** — `/api/v1/notifications/send` erzeugt minimal genau eine API-Notification, teilt denselben `data`-/`channel`-/`source`-Kompatibilitätsscope plus Integer-`priority`-Normalisierung und bleibt als expliziter Alias-Write öffentlich
